...
async function executeQuery(query) {
    const response = await fetch(`${QUESTDB_URL}?query=${encodeURIComponent(query)}`);
    if (!response.ok) {
...
export async function POST(req) {
    try {
        ...
        return new Response(JSON.stringify({ message: 'Item inserido com sucesso' }), { status: 201 });
    } catch (error) {
        return new Response(JSON.stringify({ error: error.message }), { status: 500 });
    }
}
...
export async function PUT(req) {
    ...
    try {
        ...
        await executeQuery(query);
        return new Response(JSON.stringify({ message: 'Item atualizado com sucesso' }), { status: 200 });
    } catch (error) {
        return new Response(JSON.stringify({ error: error.message }), { status: 500 });
    }
}
...
export async function DELETE(req) {
    ...
    try {
        ...
        await executeQuery(query);
        return new Response(JSON.stringify({ message: 'Item excluído com sucesso' }), { status: 204 });
    } catch (error) {
        return new Response(JSON.stringify({ error: error.message }), { status: 500 });
    }
}