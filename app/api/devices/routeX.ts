    ...
    switch (req.method) {
      ...
      case "POST":
        if (req.body.action === "register") {
          ...
          const tx = await contract.call("registerDevice", [
            name,
            location,
            status,
          ]);
          return res
            .status(200)
            .json({ message: "Device registered successfully", tx });
        }

        // Atualizar Dispositivo (PUT)
        if (req.body.action === "update") {
          const { id, name, location, status } = req.body;
          const tx = await contract.call("updateDevice", [
            id,
            name,
            location,
            status,
          ...
          return res
            .status(200)
            .json({ message: `Device ${id} updated successfully`, tx });
        }
        ...
        if (req.body.action === "delete") {
          ...
          return res
            .status(200)
            .json({ message: `Device ${id} deleted successfully`, tx });
        }

        return res
          .status(400)
          .json({ error: "Invalid action for POST/PUT/DELETE" });
